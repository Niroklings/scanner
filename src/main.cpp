...
void printUsage() {
  std::cout << "Usage: scanner --base <base.csv> --log <report.log> --path "
               "<directory>\n";
}
...
int main(int argc, char* argv[]) {
  ...
  CreateScannerFunc createScanner =
      (CreateScannerFunc)dlsym(handle, "createScanner");
  DestroyScannerFunc destroyScanner =
      (DestroyScannerFunc)dlsym(handle, "destroyScanner");
  ScanDirectoryFunc scanDirectory =
      (ScanDirectoryFunc)dlsym(handle, "scanDirectory");
  LoadHashesFunc loadHashes =
      (LoadHashesFunc)dlsym(handle, "loadMaliciousHashes");
  GetHashesCountFunc getHashesCount =
      (GetHashesCountFunc)dlsym(handle, "getMaliciousHashesCount");
