...
protected:
    void SetUp() override {
        ...
        // Создаем вредоносный файл с известным содержимым
        ...
        // Создаем CSV базу с правильным хешем
...
TEST_F(ScannerTest, ScanFindsMaliciousFiles) {
    ...
    EXPECT_GT(result.maliciousFiles, 0); // Должен найти наш файл
    EXPECT_GE(result.errorCount, 0); // Должна быть ошибка доступа
}
...
TEST_F(ScannerTest, LogFileCreation) {
    ...
    // Сканируем и проверяем что лог создался
...
TEST_F(ScannerTest, ScannerObjectLifetime) {
    ...
    // Объект должен быть корректно уничтожен
    EXPECT_TRUE(true);
}